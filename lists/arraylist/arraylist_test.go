...
import (
	...
	utils2 "github.com/lemonyxk/gods/utils"
)
...
func TestListInsert(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s", utils2.ToAny(list.Values())...), "abcd"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
}
...
func TestListSet(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s", utils2.ToAny(list.Values())...), "abbc"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
}
...
func benchmarkAdd[T comparable](b *testing.B, list *List[int], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Add(n)
		}
...
func BenchmarkArrayListRemove100(b *testing.B) {
	...
	b.StartTimer()
	benchmarkRemove[int](b, list, size)
}
