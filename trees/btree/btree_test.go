...
func TestBTreePut4(t *testing.T) {
	tree := NewWithIntComparator[int, unsafe.Pointer](3)
	assertValidTree(t, tree, 0)
...
func TestBTreeIteratorValuesAndKeys(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s%s%s%s", utils.ToAny(tree.Values())...), "xbcdefg"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
...
func assertValidTree[T comparable, P any](t *testing.T, tree *Tree[T, P], expectedSize int) {
...
func assertValidTreeNode[T comparable, P any](t *testing.T, node *Node[T, P], expectedEntries int, expectedChildren int, keys []T, hasParent bool) {
...
func benchmarkPut[T comparable, P any](b *testing.B, tree *Tree[int, struct{}], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			tree.Put(n, struct{}{})
		}
...
func benchmarkRemove[T comparable, P any](b *testing.B, tree *Tree[int, struct{}], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			tree.Remove(n)
		}
...
func BenchmarkBTreeGet100(b *testing.B) {
	...
	b.StartTimer()
	benchmarkGet[int, struct{}](b, tree, size)
}
...
func BenchmarkBTreePut100(b *testing.B) {
	...
	tree := NewWithIntComparator[int, struct{}](128)
	b.StartTimer()
	benchmarkPut[int, struct{}](b, tree, size)
}
...
func BenchmarkBTreeRemove100(b *testing.B) {
	...
	b.StartTimer()
	benchmarkRemove[int, struct{}](b, tree, size)
}
